class AddGoods < ActiveRecord::Migration
  ...
  # В случае, если миграция по тем или иным причинам не может быть выполнена доконце, процесс
  #   выкатки приложения на сервер будет прерван и приведет к повреждению данных.

  # В целях экономии времени и упращения кода я буду использовать миграцию для наполнения данных.
  def up
    goods = []
  ...
  def down
    # Наша модель goods достаточна проста и нетребовательна к качеству данных. Например, она не требует, чтобы
    #  в Базе данных хранились только уникальные названия для товаров. Это значит, что в случае ошибки во время
    #  выполнения данной транзакции и последующего ее перезапуска, часть товаров будет продублирована, но
